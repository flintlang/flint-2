...
mod ewasm_tests {
    ...
    fn test_ewasm_validity() {
        ...
        let file_names = "counter factorial shapes assert traffic_lights operators memory inits"
            .split(' ')
            .zip("Counter Factorial Shapes Assert TrafficLights Operators Memory Inits".split(' '))
            .collect::<Vec<(&str, &str)>>();
