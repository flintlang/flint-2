...
mod ewasm_tests {
    ...
    fn test_ewasm_validity() {
        ...
        let file_names = "counter factorial shapes assert traffic_lights operators memory inits rockpaperscissors public_and_visible typestates_counter"
            .split(' ')
            .zip("Counter Factorial Shapes Assert TrafficLights Operators Memory Inits RockPaperScissors MyContract Counter".split(' '))
            .collect::<Vec<(&str, &str)>>();
