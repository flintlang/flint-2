...
#### &signer Type
Libra has introduced a ```signer``` native type which stores the address of the caller of the contract. A ```signer``` type cannot be created directly in code, however, a reference to it can be received as a script argument, and the address can be accessed using the ```Signer::address_of(<signer-type>)``` method. We explicitly pass the ```&signer``` caller variable to all of the non-runtime functions in the contract, however this could be improved upon, as it is only required by some functions, for example those that require runtime checking of caller protections, or have a caller group variable. 
